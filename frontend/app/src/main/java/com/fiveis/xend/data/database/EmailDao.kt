...
interface EmailDao {
    ...
    @Query("SELECT * FROM emails WHERE sourceLabel = :label ORDER BY dateTimestamp DESC")
     ...
     */
    @Query("SELECT * FROM emails WHERE sourceLabel = 'INBOX' ORDER BY dateTimestamp DESC")
     ...
     */
    @Query("SELECT date FROM emails WHERE sourceLabel = :label ORDER BY cachedAt DESC LIMIT 1")
